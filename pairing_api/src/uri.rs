...
impl Pairing {
    ...
    pub fn create_pairing(&self, methods: Vec<Vec<String>>) -> Result<String, String> {
        let methods_str = methods
            .iter()
            .map(|method_group| format!("[{}]", method_group.join(",")))
            .collect::<Vec<_>>()
            .join(",");
        let expiry_timestamp_str = match self.params.expiry_timestamp {
        ...
        Ok(format!(
            ...
            self.topic,
            self.version,
            hex::encode(self.params.sym_key.clone()),
            methods_str,
    ...
    fn test_generate_pairing_uri() {
        let pairing = Pairing {
            ...
            params: PairingParams {
                ...
                .unwrap(),
                relay_data: None,
                expiry_timestamp: Some(1705667684), // Add an expiry timestamp for completeness
            },
        };
        ...
        let actual = pairing.create_pairing(methods).unwrap();

        let expected = "wc:c9e6d30fb34afe70a15c14e9337ba8e4d5a35dd695c39b94884b0ee60c69d168@2?\
                        symKey=7ff3e362f825ab868e20e767fe580d0311181632707e7c878cbeca0238d45b8b&\
                        methods=[wc_sessionPropose],[wc_authRequest,wc_authBatchRequest]&\
                        relay-protocol=irn&expiryTimestamp=1705667684";
