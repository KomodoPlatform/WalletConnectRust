        ...
        paste! {
            impl RelayProtocolMetadata for $param_type {
                fn irn_metadata(&self) -> IrnMetadata {
                    match self {
                        [<$param_type>]::SessionPropose(_) => $crate::rpc::params::session_propose::[<IRN_ $meta:upper _METADATA>],
                        [<$param_type>]::SessionSettle(_) => $crate::rpc::params::session_settle::[<IRN_ $meta:upper _METADATA>],
                        [<$param_type>]::SessionRequest(_) => $crate::rpc::params::session_request::[<IRN_ $meta:upper _METADATA>],
                        [<$param_type>]::SessionUpdate(_) => $crate::rpc::params::session_update::[<IRN_ $meta:upper _METADATA>],
                        [<$param_type>]::SessionDelete(_) => $crate::rpc::params::session_delete::[<IRN_ $meta:upper _METADATA>],
                        [<$param_type>]::SessionEvent(_) => $crate::rpc::params::session_event::[<IRN_ $meta:upper _METADATA>],
                        [<$param_type>]::SessionExtend(_) => $crate::rpc::params::session_extend::[<IRN_ $meta:upper _METADATA>],
                        [<$param_type>]::SessionPing(_) => $crate::rpc::params::session_ping::[<IRN_ $meta:upper _METADATA>],
                    }
                }
            }
        }
        ...
        paste! {
            impl RelayProtocolHelpers for $param_type {
                ...
                fn irn_try_from_tag(value: Value, tag: u32) -> Result<Self::Params, ParamsError> {
                    match tag {
                        tag if tag == $crate::rpc::params::session_propose::IRN_RESPONSE_METADATA.tag => {
                            Ok(Self::SessionPropose(serde_json::from_value(value)?))
                        }
                        tag if tag == $crate::rpc::params::session_settle::IRN_RESPONSE_METADATA.tag => {
                            Ok(Self::SessionSettle(serde_json::from_value(value)?))
                        }
                        tag if tag == $crate::rpc::params::session_request::IRN_RESPONSE_METADATA.tag => {
                            Ok(Self::SessionRequest(serde_json::from_value(value)?))
                        }
                        tag if tag == $crate::rpc::params::session_delete::IRN_RESPONSE_METADATA.tag => {
                            Ok(Self::SessionDelete(serde_json::from_value(value)?))
                        }
                        tag if tag == $crate::rpc::params::session_extend::IRN_RESPONSE_METADATA.tag => {
                            Ok(Self::SessionExtend(serde_json::from_value(value)?))
                        }
                        tag if tag == $crate::rpc::params::session_update::IRN_RESPONSE_METADATA.tag => {
                            Ok(Self::SessionUpdate(serde_json::from_value(value)?))
                        }
                        tag if tag == $crate::rpc::params::session_event::IRN_RESPONSE_METADATA.tag => {
